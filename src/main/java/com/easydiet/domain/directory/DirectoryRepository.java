...
public interface DirectoryRepository  extends JpaRepository<Directory, String> {
    ...
    @Query(value = "select d from Directory d where workspace_id = :workspaceId and (status = 'ENABLED' or status = 'ENABLED_IN_LIST')")
    List<Directory> listAll(String workspaceId);
